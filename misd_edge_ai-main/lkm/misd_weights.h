...
static void misd_work_fn(struct work_struct *work)
{
    ...
    if (tick_count >= DECISION_TICKS) {
        ...
        if ((temp_milli / 1000) + predicted_delta_t > 75) {
            freq_qos_update_request(&misd_qos_req, 1200000);
            pr_debug("MISD Gov: throttle — T=%d°C predicted_dt=%llu misd_avg=%llu\n",
                     temp_milli / 1000, predicted_delta_t, misd_avg);
        } else {
            freq_qos_update_request(&misd_qos_req, 1800000);
            pr_debug("MISD Gov: restore  — T=%d°C predicted_dt=%llu misd_avg=%llu\n",
                     temp_milli / 1000, predicted_delta_t, misd_avg);
        }
